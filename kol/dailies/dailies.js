...
function outputRunData(path_id, class_id)
{
	var html = "";
	
	for (var day_count = 1; day_count <= 11; day_count++)
	{
		html += "<H1>Day " + day_count + "</H1>";
		...
		for (var j = 0; j < __dailies_generation_functions.length; j++)
		{
			...
			if (result.length == 2 && result[1].length > 0)
			{
				...
				var output = result[1];
				html += "<span style=\"font-weight:bold;font-size:1.2em;\">" + identifier + ":</span> " + output + "<br>";
			}
		}
	}
