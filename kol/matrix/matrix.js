...
var __spinners = [];
var __spinner_counter = 0;
var __animation_interval = 16;
var __glyph_size = 16;
...
function matrixDrawGlyph(context, glyphs, glyph_index, x_index, y_index, alpha, colour)
{
	...
	if (false)
	{
		...
		context.fillRect(x_index * __glyph_size, y_index * __glyph_size, __glyph_size, __glyph_size);
	}
	//context.globalCompositeOperation = "lighter";
	context.globalAlpha = alpha;
	context.drawImage(glyphs, gx * __glyph_size, gy * __glyph_size, __glyph_size, __glyph_size, x_index * __glyph_size, y_index * __glyph_size, __glyph_size, __glyph_size);
	//context.globalCompositeOperation = "source-over";
...
function matrixTick()
{
	...
	var maximum_glyphs_x = Math.ceil(context_width / __glyph_size);
	var maximum_glyphs_y = Math.ceil(context_height / __glyph_size);
	...
	else
	{
		//Alpha method:
		context.fillStyle = "rgb(0,0,0)";
		context.globalAlpha = 1.0 / __glyph_size; //1.0 / (60.0);
		context.fillRect(0, 0, context_width, context_height);
	}
	...
	//Create spinners:
	var spinner_density = (context_width * context_height) / (__glyph_size * 256);
	while (__spinners.length < spinner_density)
	{
		var spinner = new Object();
		
		spinner.x_index = randomi(0, maximum_glyphs_x);
		spinner.y_index = randomi(-8, maximum_glyphs_y);
		...
		__spinners.push(spinner);
	}
	//Draw spinners:
	var spinners_next = [];
	for (var i = 0; i < __spinners.length; i++)
	{
		var spinner = __spinners[i];
	...
	__spinners = spinners_next;
	__spinner_counter++;
	
	setTimeout(function() {matrixTick()}, __animation_interval);
}
